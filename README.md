...
- TCP v6 评分页提供标准 SYN 与“SYN 载荷兼容性（实验）”的原始指标、分项得分、样本数和逐节点扣分诊断；实验采用无载荷、300、1050 字节同轮配对，默认不计综合分
- ICMP 延迟详情按丢包、P50、P95、波动率和覆盖率逐项展示原始值、分项分、权重、贡献与扣分，可直接核对各项相加是否等于最终评分
- TCP 质量页和公开接口只展示测试目标标签，不返回目录中的 IP、域名或端口
- TCP 综合分默认只纳入 ICMP 与标准 SYN；SYN 载荷兼容性保留为可选诊断权重，丢失率封顶保护、评级门槛、最少运行次数与覆盖率均可在 Emerald 设置中调整并恢复推荐值
