...
- 首页十格延迟/丢包条按“节点 + 任务”独立保留均匀样本，扩大时间范围不会因为其他节点抢占抽样名额而产生伪断点；真实无采样时段仍显示灰色
...
- TCP v6 评分页提供标准 SYN 与“SYN 载荷兼容性（实验）”的原始指标、分项得分、样本数和逐节点扣分诊断；实验采用无载荷、300、1050 字节同轮配对，推荐占综合分 10%，最多影响 10 个百分点
- ICMP 延迟详情按丢包、P50、P95、波动率和覆盖率逐项展示原始值、分项分、权重、贡献与扣分，可直接核对各项相加是否等于最终评分
- TCP 质量页和公开接口只展示测试目标标签，不返回目录中的 IP、域名或端口
- TCP 综合分默认纳入 ICMP、标准 SYN 与 SYN 载荷兼容性；载荷兼容性只占 10%，采用相对配对丢失和 P95 劣化评分，并受公共异常排除、评级门槛、最少运行次数与覆盖率保护。300/1050 字节两档必须同时有效，单档缺测只保留诊断，不按 0 分处罚
...
- “链式代理估算”以入口节点为视角，组合可选的国内接入参考、入口到落地节点 ICMP 快照与落地节点到 ChatGPT HTTPS 快照，对比不同落地节点的全链路延迟、失败率与评分
- 链式估算区分“日常复用”与“首次连接”：首次连接会为国内接入和入口到落地两段加入 TCP 与 TLS 往返参考；ChatGPT TTFB 已包含出口侧 DNS、TCP、TLS，不会重复累加
- 综合网络质量页新增多轮 TCP 检测的典型最小–最大区间、平均值及最小/平均/P50/P95/最大/首包丢失趋势，单次极端误差不会直接决定范围边界
- 解锁快照由后端分级预计算：1 小时每分钟，6–24 小时每 5 分钟，3–7 天每 15 分钟；查询不读取私有诊断载荷，访客打开页面不会产生探测
- 公开解锁接口不返回检测域名、解析地址、系统或对照 DNS、固定入口、代理地址、代理认证信息和地址指纹；NAT 节点的监测入口地址仅管理员可见，组合估算只返回任务编号、落地节点编号与 IP 协议族
