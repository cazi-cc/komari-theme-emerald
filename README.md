...
- “链式代理估算”以入口节点为视角，组合“入口到落地节点”的现有 ICMP 快照与“落地节点到 ChatGPT”的现有 HTTPS 快照，对比不同落地节点的估算延迟、失败率与评分
...
- 公开解锁接口不返回检测域名、解析地址、系统或对照 DNS、固定入口、代理地址、代理认证信息和地址指纹；组合估算只返回任务编号、落地节点编号与 IP 协议族
